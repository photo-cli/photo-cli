...
public static class CoordinateFakes
{
	...
	public static Coordinate SampleRounded(int sampleId, byte coordinatePrecision = ToolOptionFakes.CoordinatePrecisionDefault)
	{
		var coordinate = Sample(sampleId);
		return new Coordinate(Math.Round(coordinate.Latitude, coordinatePrecision), Math.Round(coordinate.Longitude, coordinatePrecision));
	}
}
